    ...
    <div class="content-hall-first">
        ...
        <div class="hall-second">
            ...
            @php
                ...
                $topFile = $completedDownloads->groupBy('file_name')->sortByDesc(fn ($items) => $items->count())->keys()->first() ?? '-';
                $directRevenue = $completedDownloads->sum('amount');
            @endphp
                ...
                <div class="downloadsales-summary">
                    ...
                    <article>
                        ...
                        <div>
                            ...
                            <strong>Rp {{ number_format($directRevenue, 0, ',', '.') }}</strong>
                        </div>
                    </article>
                </div>
                ...
                <div class="downloadsales-panel">
                    <div class="downloadsales-panel__toolbar">
                        <div class="downloadsales-search">
                            ...
                            <input id="downloadSalesSearchInput" type="search" placeholder="Search user, style, or file">
                        </div>

                        <select id="downloadSalesAccess" aria-label="Filter download status" data-datatable-filter data-table="#adminDownloadSalesTable" data-column="3">
                        ...
                        <select id="downloadmonth" aria-label="Filter date range" data-datatable-date-filter data-table="#adminDownloadSalesTable" data-column="2">
                            ...
                            <tbody>
                                @foreach($downloads as $download)
                                    <tr>
                                        ...
                                        <td class="downloadsales-row__amount">
                                            @if($download->amount > 0)
                                                <strong>Rp {{ number_format($download->amount, 0, ',', '.') }}</strong>
                                                <small>Direct sale</small>
                                            @elseif($download->status === 'Completed')
                                                <strong>Included</strong>
                                                <small>Subscription access</small>
                                            @else
                                                <strong>-</strong>
                                                <small>No revenue recorded</small>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
